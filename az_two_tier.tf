...
resource "azurerm_public_ip" "PublicIP_0" {
  ...
  # = join("", list(var.FirewallDnsName, substr(md5(azurerm_resource_group.PAN_FW_RG.id), 0, 4)))
}
...
resource "azurerm_public_ip" "PublicIP_1" {
  ...
  resource_group_name = azurerm_resource_group.PAN_FW_RG.name
  allocation_method = var.publicIPAddressType
  domain_name_label = join("", tolist([var.WebServerDnsName, substr(md5(azurerm_resource_group.PAN_FW_RG.id), 0, 4)]))
}
...
resource "azurerm_network_security_group" "PAN_FW_NSG" {
  ...
  security_rule {
    ...
    source_port_range          = "*"
    destination_port_range     = "*"
    source_address_prefix      = join("", [var.IPAddressPrefix, ".0.0/16"])
    destination_address_prefix = "*"
  }
...
resource "azurerm_route_table" "PAN_FW_RT_Trust" {
  ...
  route {
    ...
    next_hop_in_ip_address = join("", [var.IPAddressPrefix, ".2.4"])
  }
...
resource "azurerm_route_table" "PAN_FW_RT_Web" {
  ...
  route {
    ...
    address_prefix = join("", [var.IPAddressPrefix, ".4.0/24"])
    next_hop_type  = "VirtualAppliance"
    next_hop_in_ip_address = join("", [var.IPAddressPrefix, ".2.4"])
  }
...
resource "azurerm_route_table" "PAN_FW_RT_DB" {
  ...
  route {
    ...
    address_prefix = join("", [var.IPAddressPrefix, ".3.0/24"])
    next_hop_type  = "VirtualAppliance"
    next_hop_in_ip_address = join("", [var.IPAddressPrefix, ".2.4"])
  }
...
resource "azurerm_network_interface" "VNIC0_DB" {
  ...
  ip_configuration {
    ...
    private_ip_address_allocation = "Static"
    private_ip_address = "10.5.4.5"
  }

  tags = {
    displayName = "NetworkInterfaces4"
  }
}
...
resource "azurerm_virtual_machine" "PAN_FW_FW" {
  ...
  storage_os_disk {
    name          = join("", [var.FirewallVmName, "-osDisk"])
  ...
  settings = <<SETTINGS
{
  ...
  "commandToExecute": "python config_mysql.py ${azurerm_public_ip.PublicIP_1.fqdn}"
}
SETTINGS
}
  ...
  settings = <<SETTINGS
    {
      ...
      "commandToExecute": "python config-fw.py"
    }
SETTINGS
}

resource "azurerm_resource_group_template_deployment" "DBlinkedTemplate" {
  ...
  parameters_content = jsonencode({
    name = join("/", [azurerm_virtual_network.PAN_FW_VNET.name, azurerm_subnet.PAN_FW_Subnet4.name])
    location = var.location
    subnet4Prefix = join("", [var.IPAddressPrefix, ".4.0/24"])
...
resource "azurerm_resource_group_template_deployment" "WeblinkedTemplate" {
  name                = "WeblinkedTemplate"
  resource_group_name = azurerm_resource_group.PAN_FW_RG.name

  depends_on = [
    azurerm_resource_group_template_deployment.DBlinkedTemplate
  ]
  /*
  parameters = {
    name = join("/", tolist([azurerm_virtual_network.PAN_FW_VNET.name,azurerm_subnet.PAN_FW_Subnet3.name]))
    location = var.location
    subnet3Prefix = join("", tolist([var.IPAddressPrefix, ".3.0/24"]))
...
output "FirewallFQDN" {
  value = join("", ["https://", "azurerm_public_ip.PublicIP_0.fqdn"])
}

output "WebIP" {
  value = join("", ["http://", "azurerm_public_ip.PublicIP_1.ip_address"])
}

output "WebFQDN" {
  value = join("", ["http://", "azurerm_public_ip.PublicIP_1.fqdn"])
}